...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (providerId === req.userId) {
      return res
        .status(400)
        .json({ error: 'You can`t make appointment with yourself' });
    }
  ...
  async delete(req, res) {
    ...
    if (appointment.user_id !== req.userId) {
      return res.status(401).json({
        error: 'You don`t have permission to remove this appointment',
      });
    }
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error:
          'You can`t cancel this appointment anymore (Limit: 2 hours in advance)',
      });
    }
